...
import { PurchaseEntry, PurchaseEntrySchema } from './schemas/purchase-entry.schema';
...
@Module({
  ...
  imports: [MongooseModule.forFeature([{ name: PurchaseEntry.name, schema: PurchaseEntrySchema }]), ItemsModule, MongooseModule.forFeature([{ name: Supplier.name, schema: SupplierSchema }])],
})
