...
export class BackupService {
   ...
   * using the process/server local timezone (same as mark branch).
   */
  @Cron('0 5,17 * * *')
  ...
  async backupDatabase(): Promise<{ message: string; backupId: string }> {
    ...
    const dbName =
      this.connection.db.databaseName || this.connection.name || 'hms';
  ...
  async restoreDatabase(backupId: string) {
    ...
    try {
      if (fs.existsSync(folderPath) && fs.statSync(folderPath).isDirectory()) {
        ...
        this.logger.log(
          `Database restored successfully from folder ${backupId}`,
        );
