...
export class PanelsService {
    ...
    async createPanel(createPanelDto: CreatePanelDto) {
        const isExist = await this.panelModel.findOne({ name: createPanelDto.name });
        if (isExist) {
    ...
    async getPanels():Promise<string[]> {
        const panels = await this.panelModel.find({status: PanelStatus.ACTIVE}).select('name -_id').sort({_id:1}).lean().exec();
        return panels.map(panel => panel.name);
    }

    async deletePanel(name: string) {
        await this.panelModel.findOneAndUpdate({ name }, { status: PanelStatus.DELETED });
    }
